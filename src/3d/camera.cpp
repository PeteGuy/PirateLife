...
Camera::Camera()
	...
	, m_ViewAngle(90.0f)
	, m_ZClippingNear(0.001f)
	, m_ZClippingFar(1000.0f)
	, m_WindowWidth(800)
	, m_WindowHeight(600)
{
	SetPosition(0,0,0);
...
glm::mat4	Camera::getBaseView()
{
	auto basePosition = glm::vec3(0, 0, m_Position.z);
	return glm::lookAt(basePosition, basePosition + m_forwardAxis, m_upAxis);
}
...
void Camera::zoomForward()
{
	...
	SetPosition(m_Position.x+m_forwardAxis.x*5, m_Position.y + m_forwardAxis.y * 5, m_Position.z + m_forwardAxis.z * 5);
	this->UpdateProj();
	this->UpdateView();
}
